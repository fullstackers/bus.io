...
Router.prototype.getRoute = function (action) {
  ...
  var route = this.routes(Route(), action).routes(action);
  var potential = []
    .concat(this.action(wild), (action!==wild ? this.action(action) : []))
    .sort(function (a, b) { return a.index - b.index  })
    ;
  potential.forEach(function (point) {
    if (point.action === action || point.action === wild) {
...
Router.prototype.route = function (message) {
  ...
  this.getRoute(message.action()).process(message);
  return this;
};
