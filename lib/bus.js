...
function Bus (a) {
  ...
  if (!(this instanceof Bus)) return new Bus(a);
  ...
  this.onConnection = function (socket) {
    ...
    function handle (message) {
      ...
      // we have a receiver that will handle this
      self.emit('receive', message, socket);
...
Bus.prototype.receiver = function (o) {

  if (typeof o === 'object' && o instanceof MessageReceiver) {
    if (this._receiver) {
      this.removeListener('receive', this._receiver.onReceive);
      this._receiver.removeListener('error', this.onError);
    }
    return this;
  }

  if (!this._receiver || (!(this._receiver instanceof MessageReceiver))) {
    this._receiver = MessageReceiver();
    this._receiver.addListener('error', this.onError);
    this.addListener('receive', this._receiver.onReceive);
  }
...
/**
 * Binds a method to the receiver for processing the incomming messages from the exchange
 *
 * @param {function} fn
 */

Bus.prototype.in = Bus.prototype.receive = function (fn) {
