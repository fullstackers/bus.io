...
Bus.prototype.socket = function (fn) {
  var self = this;
...
Bus.prototype.alias = function (socket, name) {
  ...
  this.messageExchange().channel(name).on('message', handle);

  socket.on('disconnect', function () {
    self.messageExchange().channel(name).removeListener('message', handle);
  });
