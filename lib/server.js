...
function Server (a) {
  ...
  this.onReceivedQueue = function (message) {
    self.emit('process message', message.data);
  };
...
Server.prototype.processing = function (o) {
  ...
  if (typeof o === 'object' && o instanceof Receiver) {
    if (this._processing) {
      this.removeListener('process message', this._processing.onReceive);
    ...
    this.addListener('process message', this._processing.onReceive);
