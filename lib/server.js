...
function Server (a) {
  ...
  /**
   * Called when we receveie a message on the Queue (hack don't use this)
   *
   * @param {object} message
   * @see MessageExchange
   */
  /* WE DO NOT WANT THIS, YES IT WORKS, BUT IT IS NOT IDEAL

  this.onReceivedMessageFromQueue = function (message) {
    if (self.messages().actions().indexOf(message.data.action) < 0) {
      self.onPublish(message);
    }
  };

  this.exchange().queue().on('message', this.onReceivedMessageFromQueue);
  */
...
Server.prototype.messages = function (messages) {
  ...
  if (typeof messages !== 'undefined' && messages instanceof SocketMessages) {
  ...
  if (!this._messages) {
    this._messages = SocketMessages.make();
    this._messages.exchange().on('message', this.onMessage);
  }
...
Server.prototype.outgoing = function (o) {
  ...
  if (typeof o === 'object' && o instanceof Receiver) {
    if (this._outgoing) {
      this.removeListener('from exchange', this._outgoing.onReceive);
      this._outgoing.removeListener('error', this.onError);
      this._outgoing.removeListener('received', this.onReceivedExchange);
    }

    this._outgoing = o;
    this._outgoing.addListener('error', this.onError);
    this._outgoing.addListener('received', this.onReceivedExchange);
    this.addListener('from exchange', this._outgoing.onReceive);

    return this;
  }

  if (!this._outgoing || (!(this._outgoing instanceof Receiver))) {
    this._outgoing = Receiver();
    this._outgoing.addListener('error', this.onError);
    this._outgoing.addListener('received', this.onReceivedExchange);
    this.addListener('from exchange', this._outgoing.onReceive);
  }
