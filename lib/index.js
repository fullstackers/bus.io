...
Server.prototype.alias = function (socket, name, done) {
  ...
  this.exchange().subscribe(name, handle, function (err, channel) {
    ...
    socket.on('disconnect', function () {
      self.exchange().subscribe(name, handle, function (err, channel) {
        if (err) return self.emit('error', err);
        debug('[alias "%s"] subscribed  %s, socket %s, listeners now %s', name, channel, socket.id, self.exchange().listeners('channel ' + name).length);
      });
    });
