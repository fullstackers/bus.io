...
Route.prototype.process = function (message, done) {/* done() is for testing */
  ...
  var fns = [
    function (cb) {
      ...
      controller.on('deliver', function (message) {
        ...
        events.EventEmitter.prototype.emit.apply(self, ['deliver'].concat(args));
        self.emit('done', 'deliver', args);
      });
      controller.on('respond', function (message) {
        var args = Array.prototype.slice.call(arguments);
        events.EventEmitter.prototype.emit.apply(self, ['respond'].concat(args));
        self.emit('done', 'respond', args);
      });
      controller.on('consume', function (message) {
        var args = Array.prototype.slice.call(arguments);
        events.EventEmitter.prototype.emit.apply(self, ['consume'].concat(args));
      ...
      if (params && params.length > 1) {
        cb(null, [controller].concat(params.slice(1)));
      }
      else {
        cb(null, [controller]);
      }
  ...
  async.waterfall(fns, function (err, args) {
    if (err && err !== true) {
      ...
      events.EventEmitter.prototype.emit.apply(self, ['next'].concat(params));
      if (typeof done === 'function') {
